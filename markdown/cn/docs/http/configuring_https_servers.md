...
[](ngx_http_ssl_module.xml#ssl_protocols)和[](ngx_http_ssl_module.xml#ssl_ciphers)指令可以用来强制用户连接只能引入SSL/TLS那些强壮的协议版本和强大的加密算法。从1.0.5版本开始，nginx默认使用“`ssl_protocols SSLv3 TLSv1`”和“`ssl_ciphers HIGH:!aNULL:!MD5`”，所以只有在之前的版本，明确地配置它们才是有意义的。从1.1.13和1.0.12版本开始，nginx默认使用“`ssl_protocols SSLv3 TLSv1 TLSv1.1 TLSv1.2`”。
...
这个文件需要使用[](ngx_http_ssl_module.xml#ssl_certificate)指令来引用：
...
配置的方法是删除`ssl on`的指令，并在*:443端口添加参数`ssl`：
...
- 从0.7.14版本开始，[](ngx_http_core_module.xml#listen)指令支持`ssl`参数。
- 从0.5.32版本开始，支持SNI。
