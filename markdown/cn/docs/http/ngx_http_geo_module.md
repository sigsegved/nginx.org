...
定义从指定的变量获取客户端的IP地址。默认情况下，nginx从 `$remote_addr` 变量取得客户端IP地址，但也可以从其他变量获得（0.7.27版），比如：
...
如果使用CIDR，可以用“ `0.0.0.0/0` ”代替 `default` 。
...
> **Note:** 从1.3.0和1.2.1版本起，nginx开始支持IPv6地址。
