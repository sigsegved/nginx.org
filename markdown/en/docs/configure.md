...
After installation, the file name can always be changed in the
`nginx.conf` configuration file using the
[](ngx_core_module.xml#pid) directive.
By default the file is named
...
`nginx.conf` configuration file using the
[](ngx_core_module.xml#lock_file) directive.
...
`nginx.conf` configuration file using the
[](ngx_core_module.xml#user) directive.
...
**`--with-threads`**  
  enables the use of
[thread pools](ngx_core_module.xml#thread_pool).
**`--with-file-aio`**  
  enables the use of
[asynchronous file I/O](http/ngx_http_core_module.xml#aio)
(AIO) on FreeBSD and Linux.

**`--with-http_ssl_module`**  
  enables building a module that adds the
[HTTPS protocol support](http/ngx_http_ssl_module.html)
to an HTTP server.
...
This module is not built by default.
The [libxml2](http://xmlsoft.org) and
...
that [compresses responses](http/ngx_http_gzip_module.html)
of an HTTP server.
...
[ngx_http_index_module](http/ngx_http_index_module.html) module
...
[emits single-pixel
transparent GIF](http/ngx_http_empty_gif_module.html).
...
`--with-http_perl_module=dynamic`**  
  enables building the
[embedded Perl module](http/ngx_http_perl_module.html).
This module is not built by default.
...
After installation, the file name can always be changed in the
`nginx.conf` configuration file using the
[](http/ngx_http_log_module.xml#access_log) directive.
By default the file is named
...
**`--without-http`**  
  disables the [HTTP](http/ngx_http_core_module.html) server.
**`--without-http-cache`**  
  disables HTTP cache.
...
[mail proxy](mail/ngx_mail_core_module.html) server.
**`--with-mail_ssl_module`**  
  enables building a module that adds the
[SSL/TLS protocol support](mail/ngx_mail_ssl_module.html)
to the mail proxy server.
This module is not built by default.
The OpenSSL library is required to build and run this module.
**`--without-mail_pop3_module`**  
  disables the [POP3](mail/ngx_mail_pop3_module.html) protocol
in mail proxy server.
**`--without-mail_imap_module`**  
  disables the [IMAP](mail/ngx_mail_imap_module.html) protocol
in mail proxy server.
**`--without-mail_smtp_module`**  
  disables the [SMTP](mail/ngx_mail_smtp_module.html) protocol
in mail proxy server.
...
`--with-stream=dynamic`**  
  enables building the
[stream module](stream/ngx_stream_core_module.html)
for generic TCP/UDP proxying and load balancing.
This module is not built by default.
**`--with-stream_ssl_module`**  
  enables building a module that adds the
[SSL/TLS protocol support](stream/ngx_stream_ssl_module.html)
...
[](ngx_core_module.xml#pcre_jit) directive).
**`--without-pcre2`**  
...
The library distribution needs to be downloaded from the
[zlib](http://zlib.net) site and extracted.
The rest is done by nginx’s `./configure` and
`make`.
The library is required for the
[ngx_http_gzip_module](http/ngx_http_gzip_module.html) module.
...
**`--with-debug`**  
  enables the [debugging log](debugging_log.html).
