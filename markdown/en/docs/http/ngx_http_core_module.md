...
Read and send file operations are offloaded to threads of the specified [pool](../ngx_core_module.xml#thread_pool) . If the pool name is omitted, the pool with the name “ `default` ” is used. The pool name can also be set with variables:

```
aio threads=pool$disk;
```

By default, multi-threading is disabled, it should be enabled with the `--with-threads` configuration parameter. Currently, multi-threading is compatible only with the [epoll](../events.xml#epoll) , [kqueue](../events.xml#kqueue) , and [eventport](../events.xml#eventport) methods. Multi-threaded sending of files is only supported on Linux.
...
Delays processing of unauthorized requests with 401 response code to prevent timing attacks when access is limited by [password](ngx_http_auth_basic_module.xml) , by the [result of subrequest](ngx_http_auth_request_module.xml) , or by [JWT](ngx_http_auth_jwt_module.xml) .
...
Determines whether nginx should save the entire client request body into a file. This directive can be used during debugging, or when using the `$request_body_file` variable, or the [$r->request_body_file](ngx_http_perl_module.xml#methods) method of the module [ngx_http_perl_module](ngx_http_perl_module.xml) .
...
If the directive is specified on the [server](#server) level, the value from the default server can be used. Details are provided in the “ [Virtual server selection](server_names.xml#virtual_server_selection) ” section.
...
> **Note:** Prior to version 1.9.8, the default value was 256 on all platforms.
...
> **Note:** On systems that do not support opening of directories only for search,
to use these parameters it is required that worker processes
have read permissions for all directories being checked.

> **Note:** The [ngx_http_autoindex_module](ngx_http_autoindex_module.xml) , [ngx_http_random_index_module](ngx_http_random_index_module.xml) ,
and [ngx_http_dav_module](ngx_http_dav_module.xml) modules currently ignore this directive.
...
> **Note:** If `uri` processing leads to an error,
...
less than or equal to the time in the `If-Modified-Since` request header field.
...
- requests redirected by the [error_page](#error_page) , [index](ngx_http_index_module.xml#index) , [internal_redirect](ngx_http_internal_redirect_module.xml#internal_redirect) , [random_index](ngx_http_random_index_module.xml#random_index) , and [try_files](#try_files) directives;
- requests redirected by the `X-Accel-Redirect` response header field from an upstream server;
- subrequests formed by the
“ `include virtual` ”
command of the [ngx_http_ssi_module](ngx_http_ssi_module.xml) module, by the [ngx_http_addition_module](ngx_http_addition_module.xml) module directives, and by [auth_request](ngx_http_auth_request_module.xml#auth_request) and [mirror](ngx_http_mirror_module.xml#mirror) directives;
- requests changed by the [rewrite](ngx_http_rewrite_module.xml#rewrite) directive.
...
> **Note:** There is a limit of 10 internal redirects per request to prevent
...
> **Note:** Prior to version 1.1.18, the value `safari` matched
...
> **Note:** Prior to version 1.19.10, the default value was 100.
...
The first parameter sets a timeout during which a keep-alive client connection will stay open on the server side. The zero value disables keep-alive client connections. The optional second parameter sets a value in the `Keep-Alive: timeout=` response header field. Two parameters may differ.

The `Keep-Alive: timeout=` header field is recognized by Mozilla and Konqueror. MSIE closes keep-alive connections by itself in about 60 seconds.
...
Rate limit can also be set in the `X-Accel-Limit-Rate` header field of a proxied server response. This capability can be disabled using the [proxy_ignore_headers](ngx_http_proxy_module.xml#proxy_ignore_headers) , [fastcgi_ignore_headers](ngx_http_fastcgi_module.xml#fastcgi_ignore_headers) , [uwsgi_ignore_headers](ngx_http_uwsgi_module.xml#uwsgi_ignore_headers) , and [scgi_ignore_headers](ngx_http_scgi_module.xml#scgi_ignore_headers) directives.
...
To control closing [HTTP/2](ngx_http_v2_module.xml) connections, the directive must be specified on the [server](#server) level (1.19.1).
...
> **Note:** In versions prior to 0.8.21 this parameter is named simply `default` .

The `ssl` parameter (0.7.14) allows specifying that all connections accepted on this port should work in SSL mode. This allows for a more compact [configuration](configuring_https_servers.xml#single_http_https_server) for the server that handles both HTTP and HTTPS requests.

The `http2` parameter (1.9.5) configures the port to accept [HTTP/2](ngx_http_v2_module.xml) connections. Normally, for this to work the `ssl` parameter should be specified as well, but nginx can also be configured to accept HTTP/2 connections without SSL.

> **Note:** The parameter is deprecated,
the [http2](ngx_http_v2_module.xml#http2) directive
should be used instead.

The `quic` parameter (1.25.0) configures the port to accept [QUIC](ngx_http_v3_module.xml) connections.
...
> **Note:** The PROXY protocol version 2 is supported since version 1.13.11.
...
> **Note:** In versions prior to 0.8.21, they could only be
specified in the `listen` directive together with the `default` parameter.
...
for the listening socket (1.5.8) and [limits](https://datatracker.ietf.org/doc/html/rfc7413#section-5.1) the maximum length for the queue of connections that have not yet completed
the three-way handshake.

> **Note:** Do not enable this feature unless the server can handle
receiving the [same SYN packet with data](https://datatracker.ietf.org/doc/html/rfc7413#section-6.1) more than once.

**`backlog` = `number`**  
  sets the `backlog` parameter in the `listen()` call that limits
...
Possible values are [dataready](http://man.freebsd.org/accf_data) and [httpready](http://man.freebsd.org/accf_http) .
...
If the `setfib` , `fastopen` , `backlog` , `rcvbuf` , `sndbuf` , `accept_filter` , `deferred` , `ipv6only` , `reuseport` ,
...
> **Note:** Inappropriate use of this option may have its security [implications](http://man7.org/linux/man-pages/man7/socket.7.html) .
...
> **Note:** In versions from 0.7.1 to 0.8.41, if a request matched the prefix
...
If a location is defined by a prefix string that ends with the slash character, and requests are processed by one of [proxy_pass](ngx_http_proxy_module.xml#proxy_pass) , [fastcgi_pass](ngx_http_fastcgi_module.xml#fastcgi_pass) , [uwsgi_pass](ngx_http_uwsgi_module.xml#uwsgi_pass) , [scgi_pass](ngx_http_scgi_module.xml#scgi_pass) , [memcached_pass](ngx_http_memcached_module.xml#memcached_pass) , or [grpc_pass](ngx_http_grpc_module.xml#grpc_pass) , then the special processing is performed. In response to a request with URI equal to this string, but without the trailing slash, a permanent redirect with the code 301 will be returned to the requested URI with the slash appended. If this is not desired, an exact match of the URI and location could be defined like this:
...
Enables or disables logging of errors about not found files into [error_log](../ngx_core_module.xml#error_log) .
...
Enables or disables logging of subrequests into [access_log](ngx_http_log_module.xml#access_log) .
...
and so on.
  > **Note:** Caching of errors should be enabled separately by the [open_file_cache_errors](#open_file_cache_errors) directive.
...
> **Note:** Prior to version 1.9.5, the default value was 1 32k.
...
> **Note:** Before version 1.1.7, only a single name server could be configured.
...
> **Note:** Resolving of names into IPv6 addresses is supported
...
> **Note:** Before version 1.1.9, tuning of caching time was not possible,
and nginx always cached answers for the duration of 5 minutes.

> **Note:** To prevent DNS spoofing, it is recommended
configuring DNS servers in a properly secured trusted local network.

The optional `status_zone` parameter (1.17.1) enables [collection](ngx_http_api_module.xml#resolvers_) of DNS server statistics of requests and responses in the specified `zone` . The parameter is available as part of our [commercial subscription](https://nginx.com/products/) .
...
If the directive is set to a non-zero value, nginx will try to minimize the number of send operations on client sockets by using either NOTE_LOWAT flag of the [kqueue](../events.xml#kqueue) method or the SO_SNDLOWAT socket option. In both cases the specified `size` is used.
...
> **Note:** Before version 1.7.11, pre-loading could be enabled with `aio sendfile;` .
...
> **Note:** Prior to version 1.21.4, by default there was no limit.
...
> **Note:** Before 0.8.48, the machine’s hostname was used by default.
...
Sets the bucket size for the server names hash tables. The default value depends on the size of the processor’s cache line. The details of setting up hash tables are provided in a separate [document](../hash.xml) .
...
Sets the maximum `size` of the server names hash tables. The details of setting up hash tables are provided in a separate [document](../hash.xml) .
...
The `build` parameter (1.11.10) enables emitting a [build name](../configure.xml#build) along with nginx version.

Additionally, as part of our [commercial subscription](https://nginx.com/products/) , starting from version 1.9.13 the signature on error pages and the `Server` response header field value can be set explicitly using the `string` with variables. An empty string disables the emission of the `Server` field.
...
The directive is applicable only for subrequests with response bodies saved into memory. For example, such subrequests are created by [SSI](ngx_http_ssi_module.xml#ssi_include_set) .
...
Enables or disables the use of the TCP_NODELAY option. The option is enabled when a connection is transitioned into the keep-alive state. Additionally, it is enabled on SSL connections, for unbuffered proxying, and for [WebSocket](websocket.xml) proxying.
...
Sets the bucket size for the types hash tables. The details of setting up hash tables are provided in a separate [document](../hash.xml) .

> **Note:** Prior to version 1.5.13,
...
Sets the maximum `size` of the types hash tables. The details of setting up hash tables are provided in a separate [document](../hash.xml) .
...
Sets the bucket size for the variables hash table. The details of setting up hash tables are provided in a separate [document](../hash.xml) .
...
Sets the maximum `size` of the variables hash table. The details of setting up hash tables are provided in a separate [document](../hash.xml) .

> **Note:** Prior to version 1.5.13, the default value was 512.
...
this variable is compatible with the “ `%B` ” parameter of the `mod_log_config` Apache module
...
**`$is_request_port`**  
  “ `:` ” if [$request_port](#var_request_port) is non-empty,
...
**`$realpath_root`**  
  an absolute pathname corresponding to the [root](#root) or [alias](#alias) directive’s value
for the current request,
...
The variable’s value is made available in locations processed by the [proxy_pass](ngx_http_proxy_module.xml#proxy_pass) , [fastcgi_pass](ngx_http_fastcgi_module.xml#fastcgi_pass) , [uwsgi_pass](ngx_http_uwsgi_module.xml#uwsgi_pass) , and [scgi_pass](ngx_http_scgi_module.xml#scgi_pass) directives when the request body was read to a [memory buffer](#client_body_buffer_size) .

**`$request_body_file`**  
  name of a temporary file with the request body

At the end of processing, the file needs to be removed. To always write the request body to a file, [client_body_in_file_only](#client_body_in_file_only) needs to be enabled. When the name of a temporary file is passed in a proxied request or in a request to a FastCGI/uwsgi/SCGI server, passing the request body should be disabled by the [proxy_pass_request_body off](ngx_http_proxy_module.xml#proxy_pass_request_body) , [fastcgi_pass_request_body off](ngx_http_fastcgi_module.xml#fastcgi_pass_request_body) , [uwsgi_pass_request_body off](ngx_http_uwsgi_module.xml#uwsgi_pass_request_body) , or [scgi_pass_request_body off](ngx_http_scgi_module.xml#scgi_pass_request_body) directives, respectively.
...
**`$request_port`**  
  in this order of precedence:
port number from the [URI](https://datatracker.ietf.org/doc/html/rfc9110#section-4.2.1) authority component, or
