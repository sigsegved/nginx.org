...
> **Note:** Before version 0.9.0 only a single variable could be
specified in the first parameter.

> **Note:** Since variables are evaluated only when they are used, the mere declaration
...
Sets the bucket size for the [map](#map) variables hash tables. Default value depends on the processor’s cache line size. The details of setting up hash tables are provided in a separate [document](../hash.xml) .
...
Sets the maximum `size` of the [map](#map) variables hash tables. The details of setting up hash tables are provided in a separate [document](../hash.xml) .
