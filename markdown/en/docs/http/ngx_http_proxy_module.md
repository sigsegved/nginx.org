...
> **Note:** Before 1.7.8, the response could be cached.
...
> **Note:** As part of [commercial subscription](https://nginx.com/products/) ,
the shared memory zone also stores extended
cache [information](ngx_http_api_module.xml#http_caches_) ,
...
**`purger` = `on` | `off`**  
  Instructs whether cache entries that match a [wildcard key](#proxy_cache_purge) will be removed from the disk by the cache purger (1.7.12).
...
> **Note:** In versions 1.7.3, 1.7.7, and 1.11.10 cache header format has been changed.
...
> **Note:** This functionality is available as part of our [commercial subscription](https://nginx.com/products/) .
...
Sets the bucket `size` for hash tables used by the [proxy_hide_header](#proxy_hide_header) and [proxy_set_header](#proxy_set_header) directives. The details of setting up hash tables are provided in a separate [document](../hash.xml) .
...
Sets the maximum `size` of hash tables used by the [proxy_hide_header](#proxy_hide_header) and [proxy_set_header](#proxy_set_header) directives. The details of setting up hash tables are provided in a separate [document](../hash.xml) .
...
Sets the HTTP protocol version for proxying. By default, version 1.0 is used. Version 1.1 or 2 (1.29.4) is recommended for use with [keepalive](ngx_http_upstream_module.xml#keepalive) connections and [NTLM authentication](ngx_http_upstream_module.xml#ntlm) .
...
and `Vary` set the parameters of response [caching](#proxy_cache_valid) ;
- `X-Accel-Redirect` performs an [internal
redirect](ngx_http_core_module.xml#internal) to the specified URI;
- `X-Accel-Limit-Rate` sets the [rate
limit](ngx_http_core_module.xml#limit_rate) for transmission of a response to a client;
- `X-Accel-Buffering` enables or disables [buffering](#proxy_buffering) of a response;
- `X-Accel-Charset` sets the desired [charset](ngx_http_charset_module.xml#charset) of a response.
...
Determines whether proxied responses with codes greater than or equal to 300 should be passed to a client or be intercepted and redirected to nginx for processing with the [error_page](ngx_http_core_module.xml#error_page) directive.
...
> **Note:** This restriction does not apply to responses
that will be [cached](#proxy_cache) or [stored](#proxy_store) on disk.
...
**`denied`**  
  the server [denied](#proxy_allow_upstream) the connection (1.29.3);

> **Note:** This parameter is available as part of our [commercial subscription](https://nginx.com/products/) .
...
**`non_idempotent`**  
  normally, requests with a [non-idempotent](https://datatracker.ietf.org/doc/html/rfc7231#section-4.2.2) method
( `POST` , `LOCK` , `PATCH` )
...
The directive also defines what is considered an [unsuccessful attempt](ngx_http_upstream_module.xml#max_fails) of communication with a server. The cases of `error` , `timeout` , `denied` and `invalid_header` are always considered unsuccessful attempts, even if they are not specified in the directive. The cases of `http_500` , `http_502` , `http_503` , `http_504` , and `http_429` are considered unsuccessful attempts only if they are specified in the directive. The cases of `http_403` and `http_404` are never considered unsuccessful attempts.
...
Parameter value can contain variables. In this case, if an address is specified as a domain name, the name is searched among the described server groups, and, if not found, is determined using a [resolver](ngx_http_core_module.xml#resolver) .

A request URI is passed to the server as follows:

- If the `proxy_pass` directive is specified with a URI,
then when a request is passed to the server, the part of a [normalized](ngx_http_core_module.xml#location) request URI matching the location is replaced by a URI
...
- When the URI is changed inside a proxied location using the [rewrite](ngx_http_rewrite_module.xml#rewrite) directive,
...
[WebSocket](websocket.xml) proxying requires special configuration and is supported since version 1.3.13.
...
> **Note:** A trailer section in HTTP/1.1 is [explicitly
...
The default replacement specified by the `default` parameter uses the parameters of the [location](ngx_http_core_module.xml#location) and [proxy_pass](#proxy_pass) directives. Hence, the two configurations below are equivalent:
...
When buffering is enabled, the entire request body is [read](ngx_http_core_module.xml#client_body_buffer_size) from the client before sending the request to a proxied server.
...
When HTTP/1.1 chunked transfer encoding is used to send the original request body, the request body will be buffered regardless of the directive value unless HTTP/1.1 or HTTP/2 is [enabled](#proxy_http_version) for proxying.
...
If the directive is set to a non-zero value, nginx will try to minimize the number of send operations on outgoing connections to a proxied server by using either NOTE_LOWAT flag of the [kqueue](../events.xml#kqueue) method, or the SO_SNDLOWAT socket option, with the specified `size` .
...
Allows redefining or appending fields to the request header [passed](#proxy_pass_request_headers) to the proxied server. The `value` can contain text, variables, and their combinations. These directives are inherited from the previous configuration level if and only if there are no `proxy_set_header` directives defined on the current level.

By default, the header fields `Host` and `Connection` from the original request are not passed to the proxied server. If HTTP/1.0 or HTTP/1.1 is [enabled](#proxy_http_version) for proxying, these fields are redefined:
...
Enables saving of files to a disk. The `on` parameter saves files with paths corresponding to the directives [alias](ngx_http_core_module.xml#alias) or [root](ngx_http_core_module.xml#root) . The `off` parameter disables saving of files. In addition, the file name can be set explicitly using the `string` with variables:
