...
Defines conditions under which access to an SCGI server is allowed or [denied](#denied) . If all string parameters are not empty and not equal to “0” then the access is allowed. The conditions are evaluated each time before a connection to an SCGI server is established. Parameter values can contain variables:
...
When enabled, makes the [bind](#scgi_bind) operation at each connection attempt.
...
**`purger` = `on` | `off`**  
  Instructs whether cache entries that match a [wildcard key](#scgi_cache_purge) will be removed from the disk by the cache purger (1.7.12).
...
and `Vary` set the parameters of response [caching](#scgi_cache_valid) ;
...
Determines whether an SCGI server responses with codes greater than or equal to 300 should be passed to a client or be intercepted and redirected to nginx for processing with the [error_page](ngx_http_core_module.xml#error_page) directive.
...
that will be [cached](#scgi_cache) or [stored](#scgi_store) on disk.
...
**`denied`**  
  the server [denied](#scgi_allow_upstream) the connection (1.29.3);
...
When buffering is enabled, the entire request body is [read](ngx_http_core_module.xml#client_body_buffer_size) from the client before sending the request to an SCGI server.
...
Enables or disables creation of a separate request instance for each SCGI server. By default, a single request is used for all SCGI servers. If enabled, a separate request instance is created, allowing per-server request customization.
