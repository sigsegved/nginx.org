...
The `ngx_http_upstream_module` module is used to define groups of servers that can be referenced by the [proxy_pass](ngx_http_proxy_module.xml#proxy_pass) , [fastcgi_pass](ngx_http_fastcgi_module.xml#fastcgi_pass) , [uwsgi_pass](ngx_http_uwsgi_module.xml#uwsgi_pass) , [scgi_pass](ngx_http_scgi_module.xml#scgi_pass) , [memcached_pass](ngx_http_memcached_module.xml#memcached_pass) , and [grpc_pass](ngx_http_grpc_module.xml#grpc_pass) directives.
...
Dynamically configurable group with periodic [health checks](ngx_http_upstream_hc_module.xml) is available as part of our [commercial subscription](https://nginx.com/products/) :
...
> **Note:** If [idle keepalive](#keepalive) connections,
multiple [workers](../ngx_core_module.xml#worker_processes) ,
...
> **Note:** Since version 1.5.9 and prior to version 1.11.5,
this parameter was available as part of our [commercial subscription](https://nginx.com/products/) .

**`max_fails` = `number`**  
  sets the number of unsuccessful attempts to communicate with the server
that should happen in the duration set by the `fail_timeout` parameter to consider the server unavailable for a duration also set by the `fail_timeout` parameter.
By default, the number of unsuccessful attempts is set to 1.
The zero value disables the accounting of attempts.
What is considered an unsuccessful attempt is defined by the [proxy_next_upstream](ngx_http_proxy_module.xml#proxy_next_upstream) , [fastcgi_next_upstream](ngx_http_fastcgi_module.xml#fastcgi_next_upstream) , [uwsgi_next_upstream](ngx_http_uwsgi_module.xml#uwsgi_next_upstream) , [scgi_next_upstream](ngx_http_scgi_module.xml#scgi_next_upstream) , [memcached_next_upstream](ngx_http_memcached_module.xml#memcached_next_upstream) , and [grpc_next_upstream](ngx_http_grpc_module.xml#grpc_next_upstream) directives.
...
> **Note:** The parameter cannot be used along with the [hash](#hash) , [ip_hash](#ip_hash) , and [random](#random) load balancing methods.
...
In order for this parameter to work, the `resolver` directive must be specified in the [http](ngx_http_core_module.xml#resolver) block or in the corresponding [upstream](#resolver) block.

> **Note:** Prior to version 1.27.3, this parameter was available only as part of our [commercial subscription](https://nginx.com/products/) .

**`service` = `name`**  
  enables resolving of DNS [SRV](https://datatracker.ietf.org/doc/html/rfc2782) records and sets the service `name` (1.9.13).
...
Additionally, the following parameters are available as part of our [commercial subscription](https://nginx.com/products/) :
...
from zero to a nominal value, when unhealthy server becomes [healthy](ngx_http_upstream_hc_module.xml#health_check) ,
...
> **Note:** Prior to version 1.13.6,
the parameter could be changed only with the [API](ngx_http_api_module.xml) module.

> **Note:** If there is only a single server in a group, `max_fails` , `fail_timeout` and `slow_start` parameters
...
Additionally, as part of our [commercial subscription](https://nginx.com/products/) , such groups allow changing the group membership or modifying the settings of a particular server without the need of restarting nginx. The configuration is accessible via the [API](ngx_http_api_module.xml) module (1.13.3).

> **Note:** Prior to version 1.13.3,
the configuration was accessible only via a special location
handled by [upstream_conf](ngx_http_upstream_conf_module.xml#upstream_conf) .
...
The state is currently limited to the list of servers with their parameters. The file is read when parsing the configuration and is updated each time the upstream configuration is [changed](ngx_http_api_module.xml#http_upstreams_http_upstream_name_servers_) . Changing the file content directly should be avoided. The directive cannot be used along with the [server](#server) directive.

> **Note:** Changes made during [configuration reload](../control.xml#reconfiguration) or [binary upgrade](../control.xml#upgrade) can be lost.
...
> **Note:** IPv6 addresses are supported starting from versions 1.3.2 and 1.2.2.
...
> **Note:** Until versions 1.3.1 and 1.2.2, it was not possible to specify a weight for
...
> **Note:** It should be particularly noted that the `keepalive` directive
...
> **Note:** When using load balancing methods other than the default
...
For HTTP, the [proxy_http_version](ngx_http_proxy_module.xml#proxy_http_version) directive should be set to “ `1.1` ” or “ `2` ” and the `Connection` header field should be cleared:
...
> **Note:** Alternatively, HTTP/1.0 persistent connections can be used by passing the `Connection: Keep-Alive` header field to an upstream server,
though this method is not recommended.

For FastCGI servers, it is required to set [fastcgi_keep_conn](ngx_http_fastcgi_module.xml#fastcgi_keep_conn) for keepalive connections to work:
...
> **Note:** SCGI and uwsgi protocols do not have a notion of keepalive connections.
...
In order for NTLM authentication to work, it is necessary to enable keepalive connections to upstream servers. The [proxy_http_version](ngx_http_proxy_module.xml#proxy_http_version) directive should be set to “ `1.1` ” or “ `2` ” and the `Connection` header field should be cleared:
...
> **Note:** When using load balancer methods other than the default
round-robin method, it is necessary to activate them before
the `ntlm` directive.
...
> **Note:** Prior to version 1.11.6, incomplete requests were taken into account by default.
...
> **Note:** The `least_time` method is available as a part of our [commercial subscription](https://nginx.com/products/) .
...
The optional `status_zone` parameter (1.17.5) enables [collection](ngx_http_api_module.xml#resolvers_) of DNS server statistics of requests and responses in the specified `zone` . The parameter is available as part of our [commercial subscription](https://nginx.com/products/) .

> **Note:** Since version 1.17.5 and prior to version 1.27.3,
this directive was available only as part of our [commercial subscription](https://nginx.com/products/) .
...
> **Note:** As a load balancing method always tries to evenly distribute the load
...
with one of the following values: `Strict` , `Lax` , `None` , or
...
if the value is resolved to `Strict` , `Lax` , or `None` ,
...
The `sync` parameter (1.13.8) enables [synchronization](../stream/ngx_stream_zone_sync_module.xml#zone_sync) of the shared memory zone.
...
> **Note:** `sticky cookie` `name` [ `expires=` `time` ]
[ `domain=` `domain` ]
[ `path=` `path` ];
...
initiated by `X-Accel-Redirect` or [error_page](ngx_http_core_module.xml#error_page) ,
...
are separated by commas and colons like addresses in the [$upstream_addr](#var_upstream_addr) variable.
...
“ [$http_](ngx_http_core_module.xml#var_http_) ” prefix.
...
> **Note:** This variable is available as part of our [commercial subscription](https://nginx.com/products/) .

**`$upstream_last_server_name`**  
  keeps the name of last selected upstream server (1.25.3);
allows passing it [through SNI](ngx_http_proxy_module.xml#proxy_ssl_server_name) :
...
**`$upstream_queue_time`**  
  keeps time the request spent in the upstream [queue](#queue) (1.13.9);
the time is kept in seconds with millisecond resolution.
Times of several responses
are separated by commas and colons like addresses in the [$upstream_addr](#var_upstream_addr) variable.
