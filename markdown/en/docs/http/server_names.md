...
Server names are defined using the [server_name](ngx_http_core_module.xml#server_name) directive and determine which [server](ngx_http_core_module.xml#server) block is used for a given request. See also “ ”. They may be defined using exact names, wildcard names, or regular expressions:
...
The PCRE library supports named captures using the following syntax: If nginx fails to start and displays the error message:

```
pcre_compile() failed: unrecognized character after (?< in ...
```

this means that the PCRE library is old and the syntax “ `?P<` ” should be tried instead. The captures can also be used in digital form:
...
If it is required to process requests without the `Host` header field in a [server](ngx_http_core_module.xml#server) block which is not the default, an empty name should be specified:
...
If no [server_name](ngx_http_core_module.xml#server_name) is defined in a [server](ngx_http_core_module.xml#server) block then nginx uses the empty name as the server name.

> **Note:** nginx versions up to 0.8.48 used the machine’s hostname as the server name
in this case.
...
nginx versions up to 0.6.25 supported the special name “ `*` ” which was erroneously interpreted to be a catch-all name. It never functioned as a catch-all or wildcard server name. Instead, it supplied the functionality that is now provided by the [server_name_in_redirect](ngx_http_core_module.xml#server_name_in_redirect) directive. The special name “ `*` ” is now deprecated and the [server_name_in_redirect](ngx_http_core_module.xml#server_name_in_redirect) directive should be used. Note that there is no way to specify the catch-all name or the default server using the [server_name](ngx_http_core_module.xml#server_name) directive. This is a property of the [listen](ngx_http_core_module.xml#listen) directive and not of the [server_name](ngx_http_core_module.xml#server_name) directive. See also “ ”. It is possible to define servers listening on ports *:80 and *:8080, and direct that one will be the default server for port *:8080, while the other will be the default for port *:80:
...
Internationalized domain names ( [IDNs](https://en.wikipedia.org/wiki/Internationalized_domain_name) ) should be specified using an ASCII (Punycode) representation in the [server_name](ngx_http_core_module.xml#server_name) directive:
...
First, a connection is created in a default server context. Then, the server name can be determined in the following request processing stages, each involved in server configuration selection:
  ...
  during SSL handshake, in advance, according to [SNI](configuring_https_servers.xml#sni)
...
- in case of the [ssl_protocols](ngx_http_ssl_module.xml#ssl_protocols) directive,
...
- the [client_header_buffer_size](ngx_http_core_module.xml#client_header_buffer_size) and [merge_slashes](ngx_http_core_module.xml#merge_slashes) directives
...
- in case of the [ignore_invalid_headers](ngx_http_core_module.xml#ignore_invalid_headers) , [large_client_header_buffers](ngx_http_core_module.xml#large_client_header_buffers) ,
and [underscores_in_headers](ngx_http_core_module.xml#underscores_in_headers) directives
...
the [error_page](ngx_http_core_module.xml#error_page) directive
...
If a large number of server names are defined, or unusually long server names are defined, tuning the [server_names_hash_max_size](ngx_http_core_module.xml#server_names_hash_max_size) and [server_names_hash_bucket_size](ngx_http_core_module.xml#server_names_hash_bucket_size) directives at the *http* level may become necessary. The default value of the [server_names_hash_bucket_size](ngx_http_core_module.xml#server_names_hash_bucket_size) directive may be equal to 32, or 64, or another value, depending on CPU cache line size. If the default value is 32 and server name is defined as “ `too.long.server.name.example.org` ” then nginx will fail to start and display the error message:
...
In such a case, first try to set [server_names_hash_max_size](ngx_http_core_module.xml#server_names_hash_max_size) to a number close to the number of server names. Only if this does not help, or if nginx’s start time is unacceptably long, try to increase [server_names_hash_bucket_size](ngx_http_core_module.xml#server_names_hash_bucket_size) .
