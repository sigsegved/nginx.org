...
The `proxy_protocol` parameter (1.19.8) allows specifying that all connections accepted on this port should use the [PROXY protocol](http://www.haproxy.org/download/1.8/doc/proxy-protocol.txt) . Obtained information is passed to the [authentication server](ngx_mail_auth_http_module.xml#proxy_protocol) and can be used to [change the client address](ngx_mail_realip_module.xml) .
...
Sets the protocol for a proxied server. Supported protocols are [IMAP](ngx_mail_imap_module.xml) , [POP3](ngx_mail_pop3_module.xml) , and [SMTP](ngx_mail_smtp_module.xml) .
...
Unnecessary protocols can be disabled using the [configuration](../configure.xml) parameters `--without-mail_imap_module` , `--without-mail_pop3_module` , and `--without-mail_smtp_module` .
...
Configures name servers used to find the client’s hostname to pass it to the [authentication server](ngx_mail_auth_http_module.xml) , and in the [XCLIENT](ngx_mail_proxy_module.xml#xclient) command when proxying SMTP. For example:
...
if the passing of the [XCLIENT](ngx_mail_proxy_module.xml#xclient) command
is enabled.
