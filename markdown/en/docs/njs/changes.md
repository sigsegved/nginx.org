...
# Changes with njs 0.9.4 {#njs0.9.4}

Release Date: 28 October 2025

nginx modules:

  Feature: added HTTP forward proxy support for [ngx.fetch()](reference.xml#ngx_fetch) API.

Core:

  Bugfix: fixed [r.subrequest()](reference.xml#subrequest) to a location with JS handler for QuickJS. The bug became visible after [bellard/quickjs@42eb2795](https://github.com/bellard/quickjs/commit/42eb2795) .

# Changes with njs 0.9.3 {#njs0.9.3}
...
nginx modules:
  ...
  Bugfix: fixed heap-use-after-free in `js_set` handler used in log phase. The issue was introduced in [04f6dfb](https://github.com/nginx/njs/commit/04f6dfb) ( [0.9.2](changes.xml#njs0.9.2) ).

# Changes with njs 0.9.2 {#njs0.9.2}

Release Date: 23 September 2025

nginx modules:

  Feature: added HTTP keepalive support for [`ngx.fetch()`](reference.xml#ngx_fetch) API.

  Improvement: added configure time check when the `js_import` directive is not specified for [http](../http/ngx_http_js_module.xml#js_import) or [stream](../stream/ngx_stream_js_module.xml#js_import) .

  Bugfix: fixed merging of `js_path` directives for [http](../http/ngx_http_js_module.xml#js_path) and [stream](../stream/ngx_stream_js_module.xml#js_path) .

  Bugfix: fixed building when the [ngx_http_ssl_module](../http/ngx_http_ssl_module.xml) and [ngx_stream_ssl_module](../stream/ngx_stream_ssl_module.xml) modules are unavailable.

Core:

  Change: increased the default stack size to 160k for njs VM.

  Feature: added `njs.on('exit')` API for the [qjs](engine.xml#quickjs_engine) engine.

  Improvement: optimized memory consumption while streaming in [qjs](engine.xml#quickjs_engine) .

  Bugfix: fixed building [qjs](engine.xml#quickjs_engine) engine with clang 19.

  Bugfix: fixed building with GCC 15 and O3 optimization level.

# Changes with njs 0.9.1 {#njs0.9.1}

Release Date: 10 Jul 2025

nginx modules:

  Feature: added [Fetch API](reference.xml#ngx_fetch) for [QuickJS](engine.xml) engine.

  Feature: added state file for a [shared dictionary](../http/ngx_http_js_module.xml#js_shared_dict_zone) .

  Bugfix: fixed handling of Content-Length header when a body is provided in [Fetch API](reference.xml#ngx_fetch) .

  Bugfix: fixed [qjs](engine.xml) engine after [bellard/quickjs@458c34d2](https://github.com/bellard/quickjs/commit/458c34d) .

  Bugfix: fixed NULL pointer dereference when processing `If-Match` and `If-Unmodified-Since` headers.

Core:

  Feature: added ECDH support for [WebCrypto](reference.xml#builtin_crypto) .
...
# Changes with njs 0.9.0 {#njs0.9.0}
...
# Changes with njs 0.8.10 {#njs0.8.10}

Release Date: 08 April 2025

nginx modules:

  Feature: reading [`r.requestText`](reference.xml#r_request_text) or [`r.requestBuffer`](reference.xml#r_request_buffer) from a temporary file. Previously, an exception was thrown when accessing [`r.requestText`](reference.xml#r_request_text) or [`r.requestBuffer`](reference.xml#r_request_buffer) if the size of the client request body exceeded [`client_body_buffer_size`](../http/ngx_http_core_module.xml#client_body_buffer_size) .
  ...
  Bugfix: fixed [`incr()`](reference.xml#dict_incr) method of a shared dictionary with an empty init argument for the [QuickJS](engine.xml) engine.

  Bugfix: accepting response headers with underscore characters in [Fetch API](reference.xml#ngx_fetch) .

Core:

  Change: fixed [`serializeToString()`](reference.xml#xml_serialize_tostring) . Previously, [`serializeToString()`](reference.xml#xml_serialize_tostring) was [`exclusiveC14n()`](reference.xml#xml_exclusivec14n) which returned a string instead of Buffer. According to the published documentation, it should be [`c14n()`](reference.xml#xml_c14n) .

  Feature: added [`WebCrypto`](reference.xml#builtin_crypto) API for the [QuickJS](engine.xml) engine.

  Feature: added [`TextEncoder`](reference.xml#textencoder) and [`TextDecoder`](reference.xml#textdecoder) for the [QuickJS](engine.xml) engine.

  Feature: added [`querystring`](reference.xml#querystring) module for the [QuickJS](engine.xml) engine.

  Feature: added [`crypto`](reference.xml#crypto) module for the [QuickJS](engine.xml) engine.

  Feature: added [`xml`](reference.xml#xml) module for the [QuickJS](engine.xml) engine.

  Feature: added support for the `QuickJS-NG` library.

  Bugfix: fixed [`Buffer.concat()`](reference.xml#buffer_concat) with a single argument in [QuickJS](engine.xml) .

  Bugfix: added missed syntax error for `await` in template literal.

  Bugfix: fixed non-NULL terminated strings formatting in exceptions for the [QuickJS](engine.xml) engine.

  Bugfix: fixed compatibility with recent change in [QuickJS](engine.xml) and `QuickJS-NG` .

# Changes with njs 0.8.9 {#njs0.8.9}

Release Date: 14 January 2025

nginx modules:

  Bugfix: removed extra VM creation per server. Previously, when `js_import` was declared in [http](../http/ngx_http_js_module.xml#js_import) or [stream](../stream/ngx_stream_js_module.xml#js_import) blocks, an extra copy of the VM instance was created for each server block. This was not needed and consumed a lot of memory for configurations with many server blocks. This issue was introduced in [0.8.6](#njs0.8.6) and was partially fixed for location blocks only in [0.8.7](#njs0.8.7) .

Core:

  Feature: implemented [fs](reference.xml#njs_api_fs) for the [QuickJS](../njs/engine.xml) engine.

# Changes with njs 0.8.8 {#njs0.8.8}

Release Date: 10 December 2024

nginx modules:

  Feature: implemented [shared dictionary](reference.xml#ngx_shared) for the [QuickJS](../njs/engine.xml) engine.

  Improvement: [js_preload_object](../http/ngx_http_js_module.xml#js_preload_object) is refactored.

  Bugfix: fixed limit rated output.

  Bugfix: optimized use of SSL contexts for the [js_fetch_trusted_certificate](../http/ngx_http_js_module.xml#js_fetch_trusted_certificate) directive.

Core:

  Feature: implemented [process object](reference.xml#process) for the [QuickJS](../njs/engine.xml) engine.

  Feature: implemented the [process.kill()](reference.xml#process_kill) method.
...
# Changes with njs 0.8.7 {#njs0.8.7}

Release Date: 22 October 2024

nginx modules:

  Bugfix: eliminated unnecessary VM creation. Previously, njs consumed memory proportionally to the number of nginx locations. The issue was introduced in [0.8.6](#njs0.8.6) .

  Improvement: added strict syntax validation for [js_body_filter](../http/ngx_http_js_module.xml#js_body_filter) .

  Improvement: improved error messages for module loading failures.

Core:

  Feature: implemented [`fs.readlinkSync()`](reference.xml#fs_readlinksync) .

  Improvement: implemented lazy stack symbolization.

  Bugfix: fixed heap-buffer-overflow in `Buffer.prototype.indexOf()` . The issue was introduced in [0.8.6](#njs0.8.6) .

  Bugfix: fixed `Buffer.prototype.lastIndexOf()` when `from` is provided.

# Changes with njs 0.8.6 {#njs0.8.6}

Release Date: 02 October 2024

nginx modules:

  Feature: introduced the [QuickJS](../njs/engine.xml) engine.

  Feature: added optional `nocache` flag for the `js_set` directive for [http](../http/ngx_http_js_module.xml#js_set) and [stream](../stream/ngx_stream_js_module.xml#js_set) . Thanks to Thomas P.

  Feature: exposed [capture group variables](reference.xml#r_variables) in the HTTP module. Thanks to Thomas P.
...
# Changes with njs 0.8.5 {#njs0.8.5}
...
nginx modules:
  ...
  Change: bytes invalid in UTF-8 encoding are converted into the replacement character in:
    ...
    [`r.variables.var`](reference.xml#r_variables) , [`r.requestText`](reference.xml#r_request_text) , [`r.responseText`](reference.xml#r_response_text) , [`s.variables.var`](reference.xml#s_variables) ,

    the `data` argument of the [`s.on()`](reference.xml#s_on) callback with `upload` or `download` event types,

    the `data` argument of the [`js_body_filter`](../http/ngx_http_js_module.xml#js_body_filter) directive.

  
  When working with binary data, use:

    [`r.rawVariables.var`](reference.xml#r_raw_variables) , [`r.requestBuffer`](reference.xml#r_request_buffer) , [`r.responseBuffer`](reference.xml#r_response_buffer) , [`s.rawVariables.var`](reference.xml#s_raw_variables) ,

    the `upstream` or `downstream` event type for [`s.on()`](reference.xml#s_on) ,

    `buffer_type` = `buffer` for [`js_body_filter`](../http/ngx_http_js_module.xml#js_body_filter) .

  
  Feature: added `timeout` argument for [`add()`](reference.xml#dict_add) , [`set()`](reference.xml#dict_set) , and [`incr()`](reference.xml#dict_incr) methods of a shared dictionary.

  Bugfix: fixed checking for duplicate [`js_set`](../http/ngx_http_js_module.xml#js_set) variables.

  Bugfix: fixed request `Host` header when the port is non-standard.

  Bugfix: fixed handling of a zero-length request body in [`ngx.fetch()`](reference.xml#ngx_fetch) and [`r.subrequest()`](reference.xml#r_subrequest) .

  Bugfix: fixed heap-buffer-overflow in `Headers.get()` .

  Bugfix: fixed [`r.subrequest()`](reference.xml#r_subrequest) error handling.

Core:

  Feature: added `zlib` module for `QuickJS` engine.

  Bugfix: fixed [`zlib.inflate()`](reference.xml#zlib_inflatesync) .
...
# Changes with njs 0.8.4 {#njs0.8.4}
...
nginx modules:
  ...
  Improvement: validating URI and args arguments in [`r.subrequest()`](reference.xml#r_subrequest) .

  Improvement: checking for duplicate [js_set](../http/ngx_http_js_module.xml#js_set) variables.

  Bugfix: fixed [`clear()`](reference.xml#dict_clear) method of a shared dictionary without a timeout introduced in [0.8.3](#njs0.8.3) .

  Bugfix: fixed [`r.send()`](reference.xml#r_send) method of a shared dictionary without a timeout with `Buffer` argument.

Core:

  Feature: added `QuickJS` engine support in CLI.

  Bugfix: fixed [`atob()`](reference.xml#atob) with non-padded `base64` strings.

# Changes with njs 0.8.3 {#njs0.8.3}

Release Date: 07 February 2024

nginx modules:

  Bugfix: fixed [`Headers.set()`](reference.xml#headers_set) .

  Bugfix: fixed [js_set](../http/ngx_http_js_module.xml#js_set) with `Buffer` values.

  Bugfix: fixed [`clear()`](reference.xml#dict_clear) method of a shared dictionary when a timeout is not specified.

  Bugfix: fixed [stub_status](../http/ngx_http_stub_status_module.xml) statistics when [js_periodic](../http/ngx_http_js_module.xml#js_periodic) is enabled.
...
Core:
  ...
  Bugfix: fixed underflow in [`querystring.parse()`](reference.xml#querystring_parse) .
...
# Changes with njs 0.8.2 {#njs0.8.2}

Release Date: 24 October 2023

nginx modules:

  Feature: introduced [console](reference.xml#console) object. The following methods were introduced: [`error()`](reference.xml#console_error) , [`info()`](reference.xml#console_info) , [`log()`](reference.xml#console_log) , [`time()`](reference.xml#console_time) , [`timeEnd()`](reference.xml#console_time_end) , [`warn()`](reference.xml#console_warn) .

  Bugfix: fixed `HEAD` response handling with large Content-Length in [Fetch API](reference.xml#ngx_fetch) .

  Bugfix: fixed [`items()`](reference.xml#dict_items) method for a shared dictionary.

  Bugfix: fixed [`delete()`](reference.xml#dict_delete) method for a shared dictionary.

Core:

  Feature: extended [`fs`](reference.xml#njs_api_fs) module. Added [`fs.existsSync()`](reference.xml#fs_existssync) .

  Bugfix: fixed [`xml`](reference.xml#xml) module. Broken XML exception handling in [`xml.parse()`](reference.xml#xml_parse) method was fixed.

  Bugfix: fixed `Regexp.prototype.exec()` with global regexp and Unicode input.

# Changes with njs 0.8.1 {#njs0.8.1}

Release Date: 12 September 2023

nginx modules:

  Feature: introduced the `js_periodic` directive for [http](../http/ngx_http_js_module.xml#js_periodic) and [stream](../stream/ngx_stream_js_module.xml#js_periodic) that allows specifying a JS handler to run at regular intervals.

  Feature: implemented [`items()`](reference.xml#dict_items) method of a [shared dictionary](reference.xml#ngx_shared) . The method returns all the non-expired key-value pairs.

  Bugfix: fixed [`size()`](reference.xml#dict_size) and [`keys()`](reference.xml#dict_keys) methods of a [shared dictionary](reference.xml#ngx_shared) .

  Bugfix: fixed erroneous exception in [`r.internalRedirect()`](reference.xml#r_internal_redirect) introduced in [0.8.0](#njs0.8.0) .

Core:

  Bugfix: fixed incorrect order of keys in `Object.getOwnPropertyNames()` .

# Changes with njs 0.8.0 {#njs0.8.0}

Release Date: 06 July 2023

nginx modules:

  Change: removed special treatment of forbidden headers in [Fetch API](reference.xml#ngx_fetch) introduced in [0.7.10](#njs0.7.10) .

  Change: removed deprecated since [0.5.0](#njs0.5.0) [`r.requestBody`](reference.xml#r_request_body) and [`r.responseBody`](reference.xml#r_response_body) in [`http`](../http/ngx_http_js_module.xml) module.

  Change: throwing an exception in [`r.internalRedirect()`](reference.xml#r_internal_redirect) while filtering in [`http`](../http/ngx_http_js_module.xml) module.

  Feature: introduced more global [`nginx`](reference.xml#ngx) properties: [`ngx.build`](reference.xml#ngx_build) , [`ngx.conf_file_path`](reference.xml#ngx_conf_file_path) , [`ngx.error_log_path`](reference.xml#ngx_error_log_path) , [`ngx.prefix`](reference.xml#ngx_prefix) , [`ngx.version`](reference.xml#ngx_version) , [`ngx.version_number`](reference.xml#ngx_version_number) , and [`ngx.worker_id`](reference.xml#ngx_worker_id) .

  Feature: introduced the `js_shared_dict_zone` directive for [http](../http/ngx_http_js_module.xml#js_shared_dict_zone) and [stream](../stream/ngx_stream_js_module.xml#js_shared_dict_zone) that allows declaring a dictionary shared between worker processes.

  Feature: introduced global [`nginx.shared`](reference.xml#ngx_shared) methods and properties for working with shared dictionaries.
...
```
  ...
  Bugfix: fixed [`r.status`](reference.xml#r_status) setter when filtering in [`http`](../http/ngx_http_js_module.xml) module.

  Bugfix: fixed setting of Location header in [`http`](../http/ngx_http_js_module.xml) module.
...
Core:
  ...
  Change: non-compliant deprecated String methods were removed. The following methods were removed: [`String.bytesFrom()`](reference.xml#string_bytesfrom) , [`String.prototype.fromBytes()`](reference.xml#string_frombytes) , [`String.prototype.fromUTF8()`](reference.xml#string_fromutf8) , [`String.prototype.toBytes()`](reference.xml#string_tobytes) , [`String.prototype.toUTF8()`](reference.xml#string_toutf8) , [`String.prototype.toString(`](reference.xml#string_toutf8) .
  ...
  Feature: added [`CryptoKey`](reference.xml#cryptokey) properties in [WebCrypto API](reference.xml#builtin_crypto) . The following properties were added: [`algorithm`](reference.xml#cryptokey_alg) , [`extractable`](reference.xml#cryptokey_extractable) , [`type`](reference.xml#cryptokey_type) , [`usages`](reference.xml#cryptokey_usages) .

  Bugfix: fixed `retval` of [`сrypto.getRandomValues()`](reference.xml#crypto_get_random_values) .
...
# Changes with njs 0.7.12 {#njs0.7.12}

Release Date: 10 April 2023

nginx modules:

  Bugfix: fixed `Headers()` constructor in [Fetch API](reference.xml#ngx_fetch) .

Core:

  Feature: added [`Hash.copy()`](reference.xml#crypto_hash_copy) method in [crypto](reference.xml#crypto) module.

  Feature: added [zlib](reference.xml#zlib) module.

  Improvement: added support for `export {name as default}` statement.

  Bugfix: fixed `Number` constructor according to the spec.

# Changes with njs 0.7.11 {#njs0.7.11}
...
Core:
  ...
  Feature: added [XMLNode API](reference.xml#xml_node) to modify XML documents.
...
# Changes with njs 0.7.10 {#njs0.7.10}

Release Date: 07 February 2023

nginx modules:

  Feature: added [`Request`](reference.xml#request) , [`Response`](reference.xml#response) , and [`Headers`](reference.xml#headers) ctors in [Fetch API](reference.xml#ngx_fetch) .
...
Core:
  ...
  Feature: added [`xml`](reference.xml#xml) module for working with XML documents.

  Feature: extended support for symmetric and asymmetric keys in WebCrypto. Most notably `JWK` format for [`importKey()`](reference.xml#crypto_subtle_import_key) was added.

  Feature: extended support for symmetric and asymmetric keys in [WebCrypto API](reference.xml#builtin_crypto) . Most notably `JWK` format for [`importKey()`](reference.xml#crypto_subtle_import_key) was added. [`generateKey()`](reference.xml#crypto_subtle_generate_key) and [`exportKey()`](reference.xml#crypto_subtle_export_key) were also implemented.
...
# Changes with njs 0.7.9 {#njs0.7.9}

Release Date: 17 November 2022

nginx modules:

  Bugfix: fixed [`Fetch`](reference.xml#ngx_fetch) Response prototype reinitialization. When at least one `js_import` directive was declared in both [HTTP](../http/ngx_http_js_module.xml#js_import) and [Stream](../stream/ngx_stream_js_module.xml#js_import) , [`ngx.fetch()`](reference.xml#ngx_fetch) returned inapproriate response in Stream. The bug was introduced in [0.7.7](#njs0.7.7) .
...
Core:
  ...
  Bugfix: fixed description for [`fs.mkdir()`](reference.xml#fs_mkdirsync) and [`fs.rmdir()`](reference.xml#fs_rmdirsync) methods.
...
# Changes with njs 0.7.8 {#njs0.7.8}

Release Date: 25 October 2022

nginx modules:

  Feature: added [js_preload_object](../http/ngx_http_js_module.xml#js_preload_object) directive.

  Feature: added [`ngx.conf_prefix`](reference.xml#ngx_conf_prefix) `` property.

  Feature: added [`s.sendUpstream()`](reference.xml#s_send_upstream) and [`s.sendDownstream()`](reference.xml#s_send_downstream) in [stream](../stream/ngx_stream_js_module.xml) module.

  Feature: added support for `HEAD` method in [Fetch API](reference.xml#ngx_fetch) .

  Improvement: improved `async` callback support for [`s.send()`](reference.xml#s_send) in [stream](../stream/ngx_stream_js_module.xml) module.

Core:

  Feature: added `name` instance property for a function object.

  Feature: added [`njs.memoryStats`](reference.xml#njs_memory_stats) object.
...
# Changes with njs 0.7.7 {#njs0.7.7}
...
nginx modules:
  ...
  Feature: the number of nginx configuration contexts where js directives can be specified is extended.
    ...
    HTTP: the [js_import](../http/ngx_http_js_module.xml#js_import) , [js_path](../http/ngx_http_js_module.xml#js_path) , [js_set](../http/ngx_http_js_module.xml#js_set) , and [js_var](../http/ngx_http_js_module.xml#js_var) directives are allowed in `server` and `location` contexts. The [js_content](../http/ngx_http_js_module.xml#js_content) , [js_body_filter](../http/ngx_http_js_module.xml#js_body_filter) and [js_header_filter](../http/ngx_http_js_module.xml#js_header_filter) are allowed in [if](../http/ngx_http_rewrite_module.xml#if) context.

    Stream: the [js_import](../http/ngx_http_js_module.xml#js_import) , [js_path](../http/ngx_http_js_module.xml#js_path) , [js_set](../http/ngx_http_js_module.xml#js_set) , and [js_var](../http/ngx_http_js_module.xml#js_var) are allowed in `server` context.

  
  Feature: added [`r.internal`](reference.xml#r_internal) property.

  Bugfix: fixed reading response body in [Fetch API](reference.xml#ngx_fetch) .

  Bugfix: fixed [js_fetch_timeout](../stream/ngx_stream_js_module.xml#js_fetch_timeout) in [stream](../stream/ngx_stream_js_module.xml) .

  Bugfix: fixed socket leak with `0` fetch timeout.

Core:

  Feature: extended [`fs`](reference.xml#njs_api_fs) module. Added [`fs.openSync()`](reference.xml#fs_opensync) , [`fs.promises.open()`](reference.xml#fs_promises_open) , [`fs.fstatSync()`](reference.xml#fs_fstatsync) , [`fs.readSync()`](reference.xml#fs_readsync) , [`fs.writeSync()`](reference.xml#fs_writesync_buf) .

  The following properties of [`FileHandle`](reference.xml#fs_filehandle) are implemented: `fd` , `read()` , `stat()` , `write()` , `close()` .

  Bugfix: fixed `parseInt()` , `parseFloat()` , `Symbol.for()` with no arguments.

# Changes with njs 0.7.6 {#njs0.7.6}

Release Date: 19 July 2022

nginx modules:

  Feature: improved [`r.args{}`](reference.xml#r_args) object. Added support for multiple arguments with the same key. Added case sensitivity for keys. Keys and values are percent-decoded now.

  Bugfix: fixed [`r.headersOut{}`](reference.xml#r_headers_out) setter for special headers.

Core:

  Feature: added `Symbol.for()` and `Symbol.keyfor()` .

  Feature: added [`atob()`](reference.xml#atob) and [`btoa()`](reference.xml#btoa) from [WHATWG](https://html.spec.whatwg.org) spec.
...
# Changes with njs 0.7.5 {#njs0.7.5}
...
nginx modules:
  ...
  Bugfix: fixed [`r.headersOut{}`](reference.xml#r_headers_out) special getters when value is absent.
...
# Changes with njs 0.7.4 {#njs0.7.4}

Release Date: 24 May 2022

nginx modules:

  Feature: added extended directives for configuring [Fetch API](reference.xml#ngx_fetch) . The following directives were added for [http](../http/ngx_http_js_module.xml) and [stream](../stream/ngx_stream_js_module.xml) :

    [js_fetch_timeout](../http/ngx_http_js_module.xml#js_fetch_timeout) ,

    [js_fetch_verify](../http/ngx_http_js_module.xml#js_fetch_verify) ,

    [js_fetch_buffer_size](../http/ngx_http_js_module.xml#js_fetch_buffer_size) ,

    [js_fetch_max_response_buffer_size](../http/ngx_http_js_module.xml#js_fetch_max_response_buffer_size) .

  
  Change: [`r.internalRedirect()`](reference.xml#r_internal_redirect) now accepts escaped URIs.

  Bugfix: fixed [Response](reference.xml#response) parsing with more than 8 headers in [Fetch API](reference.xml#ngx_fetch) .

Core:

  Feature: added [`njs.version_number`](reference.xml#njs_version_number) property.

  Feature: added compatibility with BoringSSL for [WebCrypto API](reference.xml#builtin_crypto) .
...
# Changes with njs 0.7.3 {#njs0.7.3}
...
# Changes with njs 0.7.2 {#njs0.7.2}
...
# Changes with njs 0.7.1 {#njs0.7.1}

Release Date: 28 December 2021

nginx modules:

  Change: the [js_include](../http/ngx_http_js_module.xml#js_include) directive deprecated since [0.4.0](#njs0.4.0) was removed.
...
# Changes with njs 0.7.0 {#njs0.7.0}

Release Date: 19 October 2021

nginx modules:

  Feature: Added [HTTPS](../http/ngx_http_js_module.xml#js_fetch_protocols) support for [Fetch API](reference.xml#ngx_fetch) .

  Feature: Added `setReturnValue()` method for [http](reference.xml#r_set_return_value) and [stream](reference.xml#s_set_return_value) .

Core:

  Feature: introduced `Async/Await` implementation.

  Feature: added [WebCrypto API](reference.xml#builtin_crypto) implementation.
...
# Changes with njs 0.6.2 {#njs0.6.2}
...
# Changes with njs 0.6.1 {#njs0.6.1}
...
# Changes with njs 0.6.0 {#njs0.6.0}
...
# Changes with njs 0.5.3 {#njs0.5.3}

Release Date: 30 March 2021

nginx modules:

  Feature: added the `js_var` directive for [http](../http/ngx_http_js_module.xml#js_var) and [stream](../stream/ngx_stream_js_module.xml#js_var) .

# Changes with njs 0.5.2 {#njs0.5.2}

Release Date: 09 March 2021

nginx modules:

  Feature: added the [`js_body_filter`](../http/ngx_http_js_module.xml#js_body_filter) directive.

  Feature: introduced the [`s.status`](reference.xml#s_status) property for [Stream Session](reference.xml#stream) object.

Core:

  Feature: added [`njs.on`](reference.xml#njs_on) ( `exit` ) callback support.
...
# Changes with njs 0.5.1 {#njs0.5.1}

Release Date: 16 February 2021

nginx modules:

  Feature: introduced [`ngx.fetch()`](reference.xml#ngx_fetch) method implementing Fetch API.

  The following properties and methods of [`Response`](reference.xml#response) object are implemented: [`arrayBuffer()`](reference.xml#response_arraybuffer) , [`bodyUsed`](reference.xml#response_bodyused) , [`json()`](reference.xml#response_json) , [`headers`](reference.xml#response_headers) , [`ok`](reference.xml#response_ok) , [`redirect`](reference.xml#response_redirect) , [`status`](reference.xml#response_status) , [`statusText`](reference.xml#response_statustext) , [`text()`](reference.xml#response_text) , [`type`](reference.xml#response_type) , [`url`](reference.xml#response_url) .

  Notable limitations: only the `http://` scheme is supported, redirects are not handled.

  In collaboration with 洪志道 (Hong Zhi Dao).

  Feature: added the [`js_header_filter`](../http/ngx_http_js_module.xml#js_header_filter) directive.

  Bugfix: fixed processing buffered data in body filter in [stream](../stream/ngx_stream_js_module.xml) module.
...
# Changes with njs 0.5.0 {#njs0.5.0}

Release Date: 01 December 2020

nginx modules:

  Feature: introduced global [`ngx`](reference.xml#ngx) object.

  The following methods were added:

    [`ngx.log(level, message)`](reference.xml#ngx_log)
  ...
  The following properties were added:
    ...
    [`r.requestBuffer`](reference.xml#r_request_buffer) ( [`r.requestBody`](reference.xml#r_request_body) ), [`r.responseBuffer`](reference.xml#r_response_buffer) ( [`r.responseBody`](reference.xml#r_response_body) ), [`r.rawVariables`](reference.xml#r_raw_variables) ( [`r.variables`](reference.xml#r_variables) ), [`s.rawVariables`](reference.xml#s_raw_variables) ( [`s.variables`](reference.xml#s_variables) ).

  
  The following events were added in the stream module:

    [`upstream`](reference.xml#s_on) (upload), [`downstream`](reference.xml#s_on) (download).
  ...
  The following properties were added:
    ...
    [`r.requestText`](reference.xml#r_request_text) ( [`r.requestBody`](reference.xml#r_request_body) ), [`r.responseText`](reference.xml#r_response_text) ( [`r.responseBody`](reference.xml#r_response_body) ).

  
  Improvement: throwing an exception in [`r.internalRedirect()`](reference.xml#r_internal_redirect) for a subrequest.

  Bugfix: fixed promise [`r.subrequest()`](reference.xml#r_subrequest) with [`error_page`](../http/ngx_http_core_module.xml#error_page) redirect.
...
Core:
  ...
  Bugfix: fixed [`querystring.stringify()`](reference.xml#querystring_stringify) . Thanks to Artem S. Povalyukhin.

  Bugfix: fixed the catch handler for `Promise.prototype.finally()` .

  Bugfix: fixed [`querystring.parse()`](reference.xml#querystring_parse) .

# Changes with njs 0.4.4 {#njs0.4.4}
...
nginx modules:
  ...
  Bugfix: fixed [`r.httpVersion`](reference.xml#r_http_version) for HTTP/2.
...
Core:
  ...
  Feature: added support for `ArrayBuffer` in [`TextDecoder.prototype.decode()`](reference.xml#textdecoder_decode)

  Feature: added support for `Buffer` object in [`crypto`](reference.xml#crypto) methods.

  Feature: added support for `Buffer` object in [`fs`](reference.xml#njs_api_fs) methods.

  Change: [`Hash.prototype.digest()`](reference.xml#crypto_hash_digest) and [`Hmac.prototype.digest()`](reference.xml#crypto_hmac_digest) now return a `Buffer` instance instead of a byte string when encoding is not provided.

  Change: [`fs.readFile()`](reference.xml#readfilesync) and friends now return a `Buffer` instance instead of a byte string when encoding is not provided.
  ...
  Bugfix: fixed [`TextDecoder.prototype.decode()`](reference.xml#textdecoder_decode) with non-zero `TypedArray` offset.

# Changes with njs 0.4.3 {#njs0.4.3}

Release Date: 11 August 2020

Core:

  Feature: added [`Query String`](reference.xml#querystring) module.

  Feature: improved [`fs.mkdir()`](reference.xml#fs_mkdirsync) to support recursive directory creation. Thanks to Artem S. Povalyukhin.

  Feature: improved [`fs.rmdir()`](reference.xml#fs_rmdirsync) to support recursive directory removal. Thanks to Artem S. Povalyukhin.

  Feature: introduced UTF-8 decoder according to [WHATWG](https://encoding.spec.whatwg.org/) encoding spec.

  Feature: added [`TextDecoder()`](reference.xml#textdecoder) and [`TextEncoder()`](reference.xml#textencoder) implementation.
...
# Changes with njs 0.4.2 {#njs0.4.2}
...
Core:
  ...
  Feature: extended [`fs`](reference.xml#njs_api_fs) module. Added [`mkdir()`](reference.xml#fs_mkdirsync) , [`readdir()`](reference.xml#fs_readdirsync) , [`rmdir()`](reference.xml#fs_rmdirsync) , and friends.
...
# Changes with njs 0.4.1 {#njs0.4.1}

Release Date: 19 May 2020

nginx modules:

  Feature: added support for multi-value headers in [`r.headersIn{}`](reference.xml#r_headers_in) .

  Feature: introduced `raw headers` API: [`r.rawHeadersIn[]`](reference.xml#r_raw_headers_in) and [`r.rawHeadersOut[]`](reference.xml#r_raw_headers_out) .

  Feature: added [TypeScript](typescript.xml) API description.

Core:

  Bugfix: fixed `Array.prototype.slice()` for sparse arrays.

# Changes with njs 0.4.0 {#njs0.4.0}

Release Date: 23 April 2020

nginx modules:

  Feature: added support for multi-value headers in [`r.headersOut{}`](reference.xml#r_headers_out) .

  Feature: added `js_import` directive for [http](../http/ngx_http_js_module.xml#js_import) and [stream](../stream/ngx_stream_js_module.xml#js_import) .

  Improvement: improved iteration over [`r.headersOut{}`](reference.xml#r_headers_out) with special headers.

  Improvement: improved iteration over [`r.headersOut{}`](reference.xml#r_headers_out) with duplicates.

  Change: [`r.responseBody`](reference.xml#r_response_body) property handler now returns `undefined` instead of throwing an exception if the response body is not available.
...
Core:
  ...
  Bugfix: fixed reading of pseudofiles in [`fs`](reference.xml#njs_api_fs) .

# Changes with njs 0.3.9 {#njs0.3.9}

Release Date: 03 March 2020

nginx modules:

  Feature: added detached mode for [`r.subrequest()`](reference.xml#r_subrequest) . Responses to detached subrequests are ignored. Unlike ordinary subrequests, a detached subrequest can be created inside a variable handler.

Core:

  Feature: added `promises` API for [`fs`](reference.xml#njs_api_fs) module. Thanks to Artem S. Povalyukhin.

  Feature: extended [`fs`](reference.xml#njs_api_fs) module. Added [`access()`](reference.xml#fs_accesssync) , [`symlink()`](reference.xml#fs_symlinksync) , [`unlink()`](reference.xml#fs_unlinksync) , [`realpath()`](reference.xml#fs_realpathsync) , and friends. Thanks to Artem S. Povalyukhin.
  ...
  Bugfix: fixed callback invocations in [`fs`](reference.xml#njs_api_fs) module. Thanks to Artem S. Povalyukhin.
...
# Changes with njs 0.3.8 {#njs0.3.8}

Release Date: 21 January 2020

nginx modules:

  Feature: added `Promise` support for [`r.subrequest()`](reference.xml#r_subrequest) . If a callback is not provided, [`r.subrequest()`](reference.xml#r_subrequest) returns an ordinary `Promise` object that resolves to a subrequest response object.

  Change: [`r.parent`](reference.xml#r_parent) property handler now returns `undefined` instead of throwing an exception if a parent object is not available.
...
# Changes with njs 0.3.7 {#njs0.3.7}
...
# Changes with njs 0.3.6 {#njs0.3.6}

Release Date: 22 October 2019

nginx modules:

  Improvement: getting special headers from [`r.headersIn{}`](reference.xml#r_headers_in) .
...
# Changes with njs 0.3.5 {#njs0.3.5}
...
# Changes with njs 0.3.4 {#njs0.3.4}
...
Core:
  ...
  Feature: added [`fs.renameSync()`](reference.xml#fs_renamesync) .
  ...
  Bugfix: fixed reading files with zero size in [`fs.readFileSync()`](reference.xml#readfilesync) .
...
# Changes with njs 0.3.3 {#njs0.3.3}

Release Date: 25 June 2019

nginx modules:

  Improvement: getting of special response headers in [`r.headersOut{}`](reference.xml#r_headers_out) .

  Improvement: working with unknown methods in [`r.subrequest()`](reference.xml#r_subrequest) .

  Improvement: added support for null as a second argument of [`r.subrequest()`](reference.xml#r_subrequest) .
...
Core:
  ...
  Feature: added [`process`](reference.xml#process) global object.
...
# Changes with njs 0.3.2 {#njs0.3.2}
...
# Changes with njs 0.3.1 {#njs0.3.1}
...
# Changes with njs 0.3.0 {#njs0.3.0}

Release Date: 26 March 2019

nginx modules:

  Feature: added the `js_path` directive for [http](../http/ngx_http_js_module.xml#js_path) and [stream](../stream/ngx_stream_js_module.xml#js_path) .

  Change: returning undefined value instead of empty strings for absent properties in the following objects: [`r.args{}`](reference.xml#r_args) , [`r.headersIn{}`](reference.xml#r_headers_in) , [`r.headersOut{}`](reference.xml#r_headers_out) , [`r.variables{}`](reference.xml#r_variables) , [`s.variables{}`](reference.xml#s_variables) .

  Change: returning undefined value instead of throwing an exception for [`r.requestBody`](reference.xml#r_request_body) when request body is unavailable.

  Bugfix: fixed crash while iterating over [`r.args{}`](reference.xml#r_args) when a value is absent in a key-value pair.
...
Core:
  ...
  Bugfix: fixed setting length of UTF8 string in [`fs.readFileSync()`](reference.xml#readfilesync) .

  Bugfix: fixed `nxt_file_dirname()` for paths with no dir component.

# Changes with njs 0.2.8 {#njs0.2.8}
...
nginx modules:
  ...
  Feature: added support for delete operation in [`r.headersOut{}`](reference.xml#r_headers_out) .

  Feature: added support for setting nginx variables.

  Bugfix: fixed [`r.subrequest()`](reference.xml#r_subrequest) for empty body value.

  Improvement: setting special response headers in [`r.headersOut{}`](reference.xml#r_headers_out) .
...
# Changes with njs 0.2.7 {#njs0.2.7}
...
# Changes with njs 0.2.6 {#njs0.2.6}
...
Core:
  ...
  Feature: [`console.time()`](reference.xml#console_time) and [`console.timeEnd()`](reference.xml#console_time_end) methods.
...
# Changes with njs 0.2.5 {#njs0.2.5}
...
# Changes with njs 0.2.4 {#njs0.2.4}
...
nginx modules:
  ...
  New methods and properties: [`s.on()`](reference.xml#s_on) , [`s.off()`](reference.xml#s_off) , [`s.allow()`](reference.xml#s_allow) , [`s.done()`](reference.xml#s_done) , [`s.decline()`](reference.xml#s_decline) , [`s.deny()`](reference.xml#s_deny) .

  Removed properties of the [Stream](reference.xml#stream) object: `s.OK` , `s.ABORT` , `s.AGAIN` , `s.DECLINED` , `s.ERROR` (replaced with [`s.allow()`](reference.xml#s_allow) , [`s.done()`](reference.xml#s_done) , [`s.deny()`](reference.xml#s_deny) ).

  `s.buffer` (for reading replaced with data argument of the corresponding callback, for writing use [`s.send()`](reference.xml#s_send) ).

  `s.fromUpstream` (replaced with a callback for a corresponding event).

  `s.eof` (replaced with `` [`flags.last`](reference.xml#s_on_callback_last) ).
...
# Changes with njs 0.2.3 {#njs0.2.3}
...
nginx modules:
  ...
  Bugfix: getting the parent property of the main [HTTP Request](reference.xml#http) object caused a segmentation fault.
...
Core:
  ...
  Feature: [`String.bytesFrom()`](reference.xml#string_bytesfrom) method (decoding `hex` , `base64` , `base64url` into a byte string).
...
# Changes with njs 0.2.2 {#njs0.2.2}

Release Date: 19 June 2018

nginx modules:

  Change: merged HTTP `Response` and `Reply` into [HTTP Request](reference.xml#http) . New members of `Request` :
  ...
  Deprecated members of `Response` :
    ...
    `res.contentLength` (use [`req.headersOut`](reference.xml#r_headers_out) `['Content-Length']` )

    `res.contentType` (use [`req.headersOut`](reference.xml#r_headers_out) `['Content-Type']` )

  
  The deprecated properties will be removed in [next](#njs0.2.8) releases.

  Feature: HTTP [internalRedirect()](reference.xml#r_internal_redirect) method.

Core:

  Bugfix: fixed heap-buffer-overflow in `crypto.createHmac()` .

# Changes with njs 0.2.1 {#njs0.2.1}
...
nginx modules:
  ...
  Improvement: improved logging for [js_set](../http/ngx_http_js_module.xml#js_set) and [js_content](../http/ngx_http_js_module.xml#js_content) directives.

  Improvement: setting status code to 500 by default in the [js_content](../http/ngx_http_js_module.xml#js_content) handler

  Improvement: added the debug for the returned status code in [js_content](../http/ngx_http_js_module.xml#js_content) handler

  Bugfix: fixed error logging in [js_include](../http/ngx_http_js_module.xml#js_include) .
...
# Changes with njs 0.2.0 {#njs0.2.0}
...
# Changes with njs 0.1.15 {#njs0.1.15}
...
# Changes with njs 0.1.14 {#njs0.1.14}
...
# Changes with njs 0.1.13 {#njs0.1.13}
...
# Changes with njs 0.1.12 {#njs0.1.12}
...
# Changes with njs 0.1.11 {#njs0.1.11}
...
# Changes with njs 0.1.10 {#njs0.1.10}
...
# Changes with njs 0.1.7 {#njs0.1.7}

Release Date: 27 December 2016

  Change: the [js_include](../http/ngx_http_js_module.xml#js_include) directive has been disabled at server and location levels.

  Feature: exponentiation operators.

  Bugfix: miscellaneous bugs and segmentation faults have been fixed.

# Changes with njs 0.1.6 {#njs0.1.6}

Release Date: 13 December 2016

  Change: the [js_set](../http/ngx_http_js_module.xml#js_set) directive has been disabled at server and location levels.
