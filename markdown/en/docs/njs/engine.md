...
Starting from version [0.8.6](changes.xml#njs0.8.6) , multiple JavaScript engines are supported. To specify a particular engine, use the `js_engine` directive available for both the [http](../http/ngx_http_js_module.xml#js_engine) and [stream](../stream/ngx_stream_js_module.xml#js_engine) . By default, the njs engine is used.

# njs engine {#njs_engine}

njs is an embeddable JavaScript engine developed as a part of the njs module. See the [Сompatibility](compatibility.xml) section for details.
...
Since version [0.8.6](changes.xml#njs0.8.6) , a drop-in replacement for [njs/nginx objects](reference.xml) has been introduced to ensure compatibility with the njs engine, with the following exceptions:

- njs-specific API: [`njs.dump()`](reference.xml#njs_dump) , `console.dump()` .
- deprecated API: `require()` ,
use the `import` statement instead.
- `js_preload_object` directive for [http](../http/ngx_http_js_module.xml#js_preload_object) and [stream](../stream/ngx_stream_js_module.xml#js_preload_object) .
