...
[packages](../../linux_packages.xml#dynmodules) can be used:

- `nginx-module-njs` — njs
[dynamic](../ngx_core_module.xml#load_module) modules
...
The [repository](https://github.com/nginx/njs)
with njs sources can be cloned with the following command
(requires [Git](https://git-scm.com/) client):
...
[nginx](../configure.html) root directory using the
...
[dynamic](../ngx_core_module.xml#load_module):
...
To build only the njs command-line [utility](cli.html), run
