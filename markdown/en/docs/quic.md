...
Linux [binary packages](../linux_packages.html).
Please refer to the
...
Please refer to [configure.html](configure.html) for details.

The [OpenSSL](https://openssl.org) library
version 3.5.1 or higher is recommended to build nginx with QUIC support.
Otherwise, the [OpenSSL](https://openssl.org)
...
The [](http/ngx_http_core_module.xml#listen) directive in
...
To [enable](http/ngx_http_v3_module.xml#quic_retry)
address validation:
...
To [enable](http/ngx_http_ssl_module.xml#ssl_early_data)
0-RTT:
...
To [enable](http/ngx_http_v3_module.xml#quic_gso)
...
To [set](http/ngx_http_v3_module.xml#quic_host_key)
...
in the [](http/ngx_http_ssl_module.xml#ssl_protocols) directive.
...
- Build nginx with [debug support](debugging_log.html)
