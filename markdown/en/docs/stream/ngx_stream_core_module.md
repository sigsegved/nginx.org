...
for the listening socket (1.21.0) and [limits](https://datatracker.ietf.org/doc/html/rfc7413#section-5.1) the maximum length for the queue of connections that have not yet completed
...
that filters incoming connections before passing them to `accept()` (1.25.5).
...
**`bind`**  
  this parameter instructs to make a separate `bind()` call for a given address:port pair.
The fact is that if there are several `listen` directives with
the same port but different addresses, and one of the `listen` directives listens on all addresses
for the given port ( `*:` `port` ), nginx will `bind()` only to `*:` `port` .
...
> **Note:** Before version 1.25.5, different servers must listen on different `address` : `port` pairs.
...
Specifies a `size` of the [preread](stream_processing.xml#preread_phase) buffer.
...
Specifies a `timeout` of the [preread](stream_processing.xml#preread_phase) phase.
...
The optional `status_zone` parameter (1.17.1) enables [collection](../http/ngx_http_api_module.xml#resolvers_) of DNS server statistics of requests and responses in the specified `zone` . The parameter is available as part of our [commercial subscription](https://nginx.com/products/) .

> **Note:** Before version 1.11.3, this directive was available as part of our [commercial subscription](https://nginx.com/products/) .
...
**`403`**  
  access forbidden, for example, when access is limited for [certain client addresses](ngx_stream_access_module.xml)
...
**`503`**  
  service unavailable, for example, when access is limited by the [number of connections](ngx_stream_limit_conn_module.xml)
