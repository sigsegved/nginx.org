...
The `ngx_stream_keyval_module` module (1.13.7) creates variables with values taken from key-value pairs managed by the [API](../http/ngx_http_api_module.xml#stream_keyvals_) or a variable that can also be set with [njs](https://github.com/nginx/njs-examples/) .
...
Sets the `name` and `size` of the shared memory zone that keeps the key-value database. Key-value pairs are managed by the [API](../http/ngx_http_api_module.xml#stream_keyvals_) .
...
The optional `sync` parameter (1.15.0) enables [synchronization](ngx_stream_zone_sync_module.xml#zone_sync) of the shared memory zone. The synchronization requires the `timeout` parameter to be set.

> **Note:** If the synchronization is enabled, removal of key-value pairs (no matter [one](../http/ngx_http_api_module.xml#patchStreamKeyvalZoneKeyValue) or [all](../http/ngx_http_api_module.xml#deleteStreamKeyvalZoneData) )
