...
The PROXY protocol must be previously enabled by setting the `proxy_protocol` parameter in the [listen](ngx_stream_core_module.xml#listen) directive.
...
**`$proxy_protocol_tlv_aws_vpce_id`**  
  TLV value from the PROXY Protocol header representing the [ID
...
**`$proxy_protocol_tlv_azure_pel_id`**  
  TLV value from the PROXY Protocol header representing the [LinkID
...
**`$proxy_protocol_tlv_gcp_conn_id`**  
  TLV value from the PROXY Protocol header representing [Google Cloud PSC
