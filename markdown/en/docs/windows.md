...
To install nginx/Windows, [download](../download.xml) the latest mainline version distribution ( ), since the mainline branch of nginx contains all known fixes. Then unpack the distribution, go to the nginx- directory, and run `nginx` . Here is an example for the drive C: root directory:
