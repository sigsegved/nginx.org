...
שמות שרת מוגדרים על ידי המאפיין `server_name` והם קובעים איזה בלוק תצורת שרת מקובץ התצורה יהיה בשימוש לכל בקשה ובקשה לשרת. ראו גם " ". ניתן להגדירם באמצעות שמות מדוייקים, שמות Wildcard, או באמצעות ביטויים רגולריים:
...
ספריית PCRE תומכת בלכידות מבוססות שם לפי התחביר הבא: אם nginx נכשל בעלייה ומציג את הודעת השגיאה הבאה:

```
pcre_compile() failed: unrecognized character after (?< in ...
```

פירוש הדבר שספריית ה PCRE היא ישנה, ועליכם לנסות את התחביר `?P<` . את הלכידה ניתן לבצע גם בצורה ספרתית:
