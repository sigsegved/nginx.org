...
[](ngx_core_module.xml#pid), definendola all'avvio
oppure in `nginx.conf` .
Il processo master riconosce i seguenti segnali:

> **Note:** | TERM, INT | arresto rapido |
| --- | --- |
| QUIT | arresto controllato |
| HUP | ricaricamento della configurazione,
...
spegnimento controllato dei vecchi processi worker |
| USR1 | riapertura dei file di log |
| USR2 | aggiornamento del file eseguibile |
| WINCH | arresto controllato dei processi worker |
...
| QUIT | arresto controllato |
| USR1 | riapertura dei file di log |
| WINCH | chiusura anomala per debugging
(richiede [](ngx_core_module.xml#debug_points) ) |
...
ID di processo, aggiungendo il suffisso `.oldbin`, ad
esempio `/usr/local/nginx/logs/nginx.pid.oldbin`,
...
provvede a cancellare il suffisso `.oldbin` dal nome del file
