...
Per poter abilitare il log di `debug`,
...
configurare il livello di `debug`.

Si tenga presente che ridefinire il log senza anche
specificare il livello di `debug`
...
disabilita il log di `debug` per tale server:
...
esplicitamente il livello di `debug`:
...
E' pure possibile abilitare il `debug` solo
per [
indirizzi client specifici](ngx_core_module.xml#debug_connection):
