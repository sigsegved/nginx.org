# Installare nginx

**Translator:** Angelo Papadia  
**Revision:** 3  
**Language:** it

nginx puo' essere installato in varie maniere, a seconda del sistema operativo.

# Installazione su Linux {#linux}

Con Linux, e' possibile usare i [pacchetti](../linux_packages.xml) nginx binari, scaricabili da nginx.org.

# Installazione su FreeBSD {#freebsd}

Con FreeBSD, e' possibile installare nginx o tramite [pacchetti](https://docs.freebsd.org/en/books/handbook/ports/#pkgng-intro) binari oppure tramite [ports](https://docs.freebsd.org/en/books/handbook/ports/#ports-using) . Ports consente maggiore flessibilita', e permette di scegliere fra un gran numero di opzioni: ports provvedera' a compilare nginx con le opzioni specificate, e ad installarlo.

# Compilazione dai sorgenti {#source_install}

Se e' richiesta qualche funzionalita' particolare e non disponibile con i pacchetti binari o con ports, e' possibile compilare nginx a partire dai sorgenti. Per quanto maggiormente flessibile, questo approccio per un principiante puo' risultare piu' complesso. Per maggiori informazioni, fare riferimento a .

