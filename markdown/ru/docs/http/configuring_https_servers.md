...
`ssl` на
[слушающих сокетах](ngx_http_core_module.xml#listen)
в блоке [](ngx_http_core_module.xml#server),
а также указать местоположение файлов с
[сертификатом сервера](ngx_http_ssl_module.xml#ssl_certificate)
и
[секретным ключом](ngx_http_ssl_module.xml#ssl_certificate_key):
...
С помощью директив [](ngx_http_ssl_module.xml#ssl_protocols) и
...
с помощью директивы [](ngx_http_ssl_module.xml#ssl),
что не позволяло настроить единый HTTP/HTTPS сервер.
Для решения этой задачи был добавлен
параметр `ssl`
директивы [](ngx_http_core_module.xml#listen).
Поэтому использование
директивы [](ngx_http_ssl_module.xml#ssl)
...
IP-адресе —
[расширение
Server Name Indication протокола TLS](http://en.wikipedia.org/wiki/Server_Name_Indication) (SNI, RFC 6066),
...
Сейчас SNI
[поддерживается](http://en.wikipedia.org/wiki/Server_Name_Indication#Support)
