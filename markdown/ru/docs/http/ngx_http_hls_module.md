...
Модуль `ngx_http_hls_module` обеспечивает серверную поддержку
протокола HTTP Live Streaming (HLS) для медиафайлов в формате MP4 и MOV.
Такие файлы обычно имеют расширения `.mp4`, `.m4v`,
`.m4a`, `.mov` или `.qt`.
...
В такой конфигурации для файла “`/var/video/test.mp4`”
...
Задаёт максимальное число и размер буферов,
...
ngx_http_secure_link_module,
переменную $uri не следует использовать в выражении
,
так как это приведёт к ошибкам при запросах к фрагментам.
Вместо $uri следует использовать
базовую часть URI
($hls_uri в примере):
...
Задаёт начальный размер буфера, используемого для
