...
заранее [
подготовить исходный файл для псевдо-стриминга](https://github.com/flowplayer/flowplayer/wiki/7.1.1-video-file-correction),
...
Если аргумент `start` указывает на
...
ближайший к точке `start` ключевой кадр
...
[ngx_http_flv_module](ngx_http_flv_module.html).
...
Если аргумент start не указывает на ключевой кадр,
