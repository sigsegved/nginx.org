...
Модуль `ngx_http_scgi_module` позволяет передавать
...
директивы scgi_bind, позволяя системе
...
Директива scgi_hide_header задаёт дополнительные поля,
...
Задаёт параметр, который будет передаваться SCGI-серверу.
