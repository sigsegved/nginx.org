...
```
**`timefmt`**  
  строка формата, передаваемая функции strftime
...
**`encoding`**  
  способ кодирования.
Возможны три значения — `none`, `url` и
`entity`.
...
и [ngx_http_scgi_module](ngx_http_scgi_module.html) (1.5.6).
...
и [](ngx_http_scgi_module.xml#scgi_buffer_size).
**`set`**  
...
Формат задаётся командой `config`
