...
[группе](ngx_http_upstream_module.xml#upstream),
указанной в содержащем location.
Группа должна находиться в
[зоне разделяемой памяти](ngx_http_upstream_module.xml#zone).
...
то можно использовать необязательный параметр grpc_status
для указания
статуса
(например
...
grpc_service и grpc_status
должны быть указаны после type=grpc.
