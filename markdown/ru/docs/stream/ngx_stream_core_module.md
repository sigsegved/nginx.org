...
Задаёт адрес и порт для сокета,
...
Параметр udp конфигурирует слушающий сокет
...
Параметр proxy_protocol (1.11.4)
...
(параметр TCP_DEFER_ACCEPT сокета) на Linux (1.25.5).
...
и name-based (через
расширение
Server Name Indication протокола TLS (SNI, RFC 6066)) (1.25.5) нет.
...
Если параметр директивы установлен в “$hostname”, то
...
Модуль `ngx_stream_core_module` поддерживает переменные
...
заголовок протокола PROXY
**`403`**  
  доступ запрещён, например при ограничении доступа для
[определённых адресов клиентов](ngx_stream_access_module.html)
**`500`**  
...
[числу соединений](ngx_stream_limit_conn_module.html)
***$time_iso8601***  
