...
Модуль `ngx_stream_js_module` позволяет задавать обработчики на [njs](../njs/index.xml) — подмножестве языка JavaScript.
...
Задаёт функцию njs, которая будет вызываться в [access](stream_processing.xml#access_phase) -фазе. Начиная с [0.4.0](../njs/changes.xml#njs0.4.0) можно ссылаться на функцию модуля.

Функция вызывается однократно при первом достижении сессией [access](stream_processing.xml#access_phase) -фазы. Функция вызывается со следующими аргументами:

**`s`**  
  объект [stream-сессии](../njs/reference.xml#stream)

В этой фазе может происходить инициализация, также при помощи метода [`s.on()`](../njs/reference.xml#s_on) может регистрироваться вызов для каждого входящего блока данных пока не будет вызван один из методов: [`s.done()`](../njs/reference.xml#s_done) [`s.decline()`](../njs/reference.xml#s_decline) , [`s.allow()`](../njs/reference.xml#s_allow) . При вызове любого из этих методов обработка сессии переходит на [следующую фазу](stream_processing.xml) и все текущие вызовы [`s.on()`](../njs/reference.xml#s_on) сбрасываются.
...
Задаёт максимальное число контекстов JS для повторного использования [движке QuickJS](../njs/engine.xml) . Каждый контекст используется для одной stream-сессии. Завершённый контекст помещается в пул повторно используемых контекстов. Если пул заполнен, контекст уничтожается.
...
Описывает разрешённые шифры для HTTPS-соединений при помощи [Fetch API](../njs/reference.xml#ngx_fetch) . Шифры задаются в формате, поддерживаемом библиотекой OpenSSL.
...
Разрешает указанные протоколы для HTTPS-соединений при помощи [Fetch API](../njs/reference.xml#ngx_fetch) .
...
Задаёт фильтр данных. Начиная с [0.4.0](../njs/changes.xml#njs0.4.0) можно ссылаться на функцию модуля. Функция фильтра вызывается однократно при первом достижении сессией [content](stream_processing.xml#content_phase) -фазы.
...
В этой фазе может происходить инициализация, также при помощи метода [`s.on()`](../njs/reference.xml#s_on) может регистрироваться вызов для каждого входящего блока данных. Для отмены регистрации вызова и отмены фильтра можно использовать метод [`s.off()`](../njs/reference.xml#s_off) .

> **Note:** Так как обработчик `js_filter` должен сразу возвращать результат,
то поддерживаются только синхронные операции.
Таким образом, асинхронные операции, например [`ngx.fetch()`](../njs/reference.xml#ngx_fetch) или [`setTimeout()`](../njs/reference.xml#settimeout) ,
не поддерживаются.
...
на уровне `server` начиная с [0.7.7](../njs/changes.xml#njs0.7.7) .
...
Задаёт функцию njs, которая будет вызываться в [preread](stream_processing.xml#preread_phase) -фазе. Начиная с [0.4.0](../njs/changes.xml#njs0.4.0) можно ссылаться на функцию модуля.

Функция вызывается однократно при первом достижении сессией [preread](stream_processing.xml#preread_phase) -фазы. Функция вызывается со следующими аргументами:
...
> **Note:** Так как обработчик `js_preread` должен сразу возвращать результат,
то поддерживаются только синхронные операции.
Таким образом, асинхронные операции, например [`ngx.fetch()`](../njs/reference.xml#ngx_fetch) или [`setTimeout()`](../njs/reference.xml#settimeout) ,
не поддерживаются.
Тем не менее асинхронные операции поддерживаются в вызовах [`s.on()`](../njs/reference.xml#s_on) в [preread](stream_processing.xml#preread_phase) -фазе.
Подробнее см. [пример](https://github.com/nginx/njs-examples#authorizing-connections-using-ngx-fetch-as-auth-request-stream-auth-request) .
...
Функция вызывается в момент первого обращения к переменной для данного запроса. Точный момент вызова функции зависит от [фазы](stream_processing.xml) , в которой происходит обращение к переменной. Это можно использовать для реализации дополнительной логики, не относящейся к вычислению переменной. Например, если переменная указана в директиве [log_format](ngx_stream_log_module.xml#log_format) , то её обработчик не будет выполняться до фазы записи в лог. Этот обработчик также может использоваться для выполнения процедур непосредственно перед освобождением запроса.

Начиная с [0.8.6](../njs/changes.xml#njs0.8.6) , если указан необязательный параметр `nocache` , то обработчик выполняется каждый раз при обращении к переменной. Из-за ограничения модуля [rewrite](ngx_stream_rewrite_module.xml) при обращении к `nocache` -переменной при помощи директивы [set](ngx_stream_rewrite_module.xml#set) , обработчик должен возвращать значение фиксированной длины.
...
Объявляет [перезаписываемую](../njs/reference.xml#r_variables) переменную. В качестве значения можно использовать текст, переменные и их комбинации.
...
Каждый stream-обработчик njs получает один аргумент, [объект](../njs/reference.xml#stream) stream-сессии.
