...
Модуль `ngx_stream_log_module` (1.11.4) записывает логи сессий
в указанном формате.
...
указан параметр gzip, то запись будет
буферизованной.
...
Параметр if включает условную запись в лог.
...
Параметр none (1.13.10) отключает
