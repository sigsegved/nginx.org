...
Модуль `ngx_stream_ssl_module` (1.9.0) обеспечивает необходимую поддержку для работы прокси-сервера по протоколу SSL/TLS.
...
Задаёт список поддерживаемых протоколов [ALPN](https://datatracker.ietf.org/doc/html/rfc7301) . Один из протоколов должен быть [согласован](#var_ssl_alpn_protocol) , если клиент использует ALPN:
...
Для преобразования имени хоста OCSP responder’а в адрес необходимо дополнительно задать директиву [resolver](ngx_stream_core_module.xml#resolver) .
...
Если директива указана на уровне [server](ngx_stream_core_module.xml#server) , то может использоваться значение из сервера по умолчанию. Подробнее см. в разделе “ [Выбор виртуального сервера](../http/server_names.xml#virtual_server_selection) ”.

> **Note:** Параметры `TLSv1.1` и `TLSv1.2` работают только при использовании OpenSSL 1.0.1 и выше.
...
Если разрешено, то операции SSL handshake в блоке [server](ngx_stream_core_module.xml#server) будут отклонены.
...
Разрешает проверку клиентских сертификатов. Результат проверки доступен через переменную [$ssl_client_verify](#var_ssl_client_verify) . Если при проверке клиентского сертификата произошла ошибка или клиент не предоставил требуемый сертификат, соединение закрывается.
...
для установленного SSL-соединения согласно [RFC 2253](https://datatracker.ietf.org/doc/html/rfc2253) (1.11.8);
