...
Модуль `ngx_stream_upstream_hc_module` (1.9.0)
позволяет активировать периодические проверки работоспособности серверов в
[группе](ngx_stream_upstream_module.xml#upstream).
Группа должна находиться в
[зоне разделяемой памяти](ngx_stream_upstream_module.xml#zone).
...
Тесты настраиваются отдельно при помощи директивы 
...
для UDP проверяется отсутствие
сообщения ICMP “Destination Unreachable”
в ответ на отправленную строку “nginx health check”.

До версии 1.11.7 по умолчанию для UDP
требовалось наличие блока match с параметрами
send и expect.
...
UDP
вместо протокола TCP, используемого по умолчанию (1.9.13).
