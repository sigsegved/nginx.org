...
- `-c` —использование альтернативного
конфигурационного файла `файл` вместо файла по умолчанию.
- `-e` —использование альтернативного
лог-файла ошибок `файл` вместо файла по умолчанию (1.19.5).
Специальное значение `stderr` выбирает стандартный файл ошибок.
- `-g` —задание [глобальных директив конфигурации](ngx_core_module.xml) ,
например
  ```
nginx -g "pid /var/run/nginx.pid; worker_processes `sysctl -n hw.ncpu`;"
```

- `-p` —задание префикса пути nginx,
