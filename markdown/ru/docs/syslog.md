...
Директивы [error_log](ngx_core_module.xml#error_log) и [access_log](http/ngx_http_log_module.xml#access_log) поддерживают запись в syslog. Запись в syslog настраивается при помощи следующих параметров:
...
**`facility=` `строка`**  
  Задаёт категорию сообщений syslog в соответствии с [RFC 3164](https://datatracker.ietf.org/doc/html/rfc3164#section-4.1.1) .
...
**`severity=` `строка`**  
  Задаёт важность сообщений syslog для [access_log](http/ngx_http_log_module.xml#access_log) в соответствии с [RFC 3164](https://datatracker.ietf.org/doc/html/rfc3164#section-4.1.1) .
Возможны те же самые значения, что и у второго параметра (уровень)
директивы [error_log](ngx_core_module.xml#error_log) .
...
Как часть [коммерческой подписки](https://nginx.com/products/) запись в syslog доступна начиная с версии 1.5.3.
