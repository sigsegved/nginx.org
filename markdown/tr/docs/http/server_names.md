...
Sunucu adları `server_name` yönergesi kullanılarak tanımlanırlar ve gelen bir talep için hangi server bloğunun kullanılacağını belirlerler. Ayrıca bakınız "[request_processing.html](request_processing.html)". Gerçek, wildcard veya düzenli ifadeler şeklinde tanımlanabilirler.
...
bu PCRE kütüphanesini eski olduğu ve `?P<name>` şeklinde kullanmanız gerektiği anlamına gelir.
...
Fakat bu bir catch-all veya wildcard sunucu adı olarak fonksiyonel değil. Bunun yerine, `server_name_in_redirect` yönergesini kullanarak fonksiyonelliği sağlamaya başladık.
"*" özel karakteri artık desteklenmiyor, bu yüzden `server_name_in_redirect` yönergesi kullanılmalıdır.
Not: `server_name` yönergesini kullanan varsayılan sunucuyu veya catch-all adını belirtmenin bir yolu bulunmuyor.
Bu, `server_name` yönergesinin değil, `listen` yönergesinin bir özelliğidir.
Ayrıca bakınız "[request_processing.html](request_processing.html)".
